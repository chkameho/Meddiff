...
class count_tool:
    ...
    def sum_of_leucocyte(self): 
        ...
        return sum(st.session_state[self.count_times]["leucocyte_count"].values())
    
    def add_or_sub_one(self, add_or_sub_count):
        """
    ...
    def one_column_col(self, list_of_input, variable):
        """
    ...
    def HemaDiff_tool(self, add_or_sub_count):
        """
        ...
        count = self.sum_of_leucocyte()

        variable = self.add_or_sub_one(add_or_sub_count)

        col1, col2, col3, col4 = st.columns(4, gap="small")

        if count <= 99:
            with col1:
                self.one_column_col(['Basophil','Monozyt','Blast','A'], variable = variable)
            with col2: 
                self.one_column_col(['Eosinophil','Lymphozyt','Promyelozyt','B'], variable = variable)
            with col3: 
